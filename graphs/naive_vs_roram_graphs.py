...
def plot_data():
    ...
    plt.xlabel("R")
    plt.ylabel("Time (s)")
    plt.plot(rs[:len(naive_access_data)], naive_access_data, label="Naive") 
    plt.plot(rs[:len(roram_access_data)], roram_access_data, label="RORAM")
    plt.legend()
    plt.ylim(ymin=0)

    plt.subplot(1, 6, 2)
    plt.title("# of seeks vs r for Naive Range ORAM and RORAM")
    plt.xlabel("R")
    plt.ylabel("Seeks")
    plt.plot(rs[:len(naive_seeks_data)], naive_seeks_data, label="Naive") 
    plt.plot(rs[:len(roram_seeks_data)], roram_seeks_data, label="RORAM")
    plt.legend()
    plt.ylim(ymin=0)
    ...
    plt.ylabel("Time (s)")
    plt.plot(rs[:len(naive_access_data)], naive_access_data, label="Naive") 
    plt.plot(rs[:len(naive_access_data)], roram_access_data[:len(naive_access_data)], label="RORAM")
    plt.legend()
    plt.ylim(ymin=0)
    ...
    plt.ylabel("Seeks")
    plt.plot(rs[:len(naive_seeks_data)], naive_seeks_data, label="Naive") 
    plt.plot(rs[:len(naive_seeks_data)], roram_seeks_data[:len(naive_seeks_data)], label="RORAM")
    plt.legend()
    plt.ylim(ymin=0)
